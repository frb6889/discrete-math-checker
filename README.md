...
2.找路，最小的容量就是该路流量，加
3.继续找路，直到无路可走
（注意：有时随机找到的情况不是最优情况，记得自己手动多跑点小数据，，）
...
- **所有元素都可逆**，**即对于任意a∈G,都有aa^-1=a^-1a=e** 这是幺群和群的区别
...
###### 8.3 循环群和群的同构
证明两群同构：
- 1.同类型代数系统，找到一个一对一的映射
- 2.同类型代数系统，先证双射，再证对任意a,b∈G有f(ab)=f(a)·f(b)
...
**变换群与置换群：**
- **变换群**：指所有可能对集合进行的变换（或置换）所形成的群。
- **置换群**：由一个有限集合上所有置换组成的群，通常表示为 \( S_n \)，其中 \( n \) 是集合中元素的数量。

**Cayley定理：**
- **定义**：每个群 \( G \) 都同构于某个置换群的一个子群。具体地说，可以将 \( G \) 中的每个元素 \( g \) 对应到 \( G \) 上的左正则变换（左乘变换），形成一个同构。

###### 8.5 陪集和群的陪集分解 Lagrange定理

**陪集：**
- **左陪集** \( gH \)：对于群 \( G \) 的子群 \( H \)，包含形如 \( gh \) 的所有元素，其中 \( h \in H \)。
- **右陪集** \( Hg \)：包含形如 \( hg \) 的所有元素，其中 \( h \in H \)。

**Lagrange定理：**
- **定义**：对于有限群 \( G \) 和其子群 \( H \)，\( |G| \)（\( G \) 的阶）等于 \( H \) 的阶 \( |H| \) 乘以 \( G \) 中每个左陪集的个数。数学表达为：
\[ |G| = |H| \cdot [G : H] \]
其中 \( [G : H] \) 表示 \( G \) 中左陪集的数目，也等于右陪集的数目。
